		...
		{foreach from=$aStreamEvents item=oStreamEvent}		
			...
			<li class="stream-item-type-{$oStreamEvent->getEventType()}">
				...
				{if $oStreamEvent->getEventType() == 'add_topic'}
					{$aLang.stream_list_event_add_topic} <a href="{$oTarget->getUrl()}" class="act">{$oTarget->getTitle()|escape:'html'}</a>
				{elseif $oStreamEvent->getEventType() == 'add_comment'}
					{$aLang.stream_list_event_add_comment} <a href="{$oTarget->getTarget()->getUrl()}#comment{$oTarget->getId()}" class="act">{$oTarget->getTarget()->getTitle()|escape:'html'}</a>
					<div class="stream-comment-preview">{$oTarget->getText()|strip_tags|truncate:200}</div>
				{elseif $oStreamEvent->getEventType() == 'add_blog'}
					{$aLang.stream_list_event_add_blog} <a href="{$oTarget->getUrlFull()}" class="act">{$oTarget->getTitle()|escape:'html'}</a>
				{elseif $oStreamEvent->getEventType() == 'vote_blog'}
					{$aLang.stream_list_event_vote_blog} <a href="{$oTarget->getUrlFull()}" class="act">{$oTarget->getTitle()|escape:'html'}</a>
				{elseif $oStreamEvent->getEventType() == 'vote_topic'}
					{$aLang.stream_list_event_vote_topic} <a href="{$oTarget->getUrl()}" class="act">{$oTarget->getTitle()|escape:'html'}</a>
				{elseif $oStreamEvent->getEventType() == 'vote_comment'}
					{$aLang.stream_list_event_vote_comment} <a href="{$oTarget->getTarget()->getUrl()}#comment{$oTarget->getId()}" class="act">{$oTarget->getTarget()->getTitle()|escape:'html'}</a>
				{elseif $oStreamEvent->getEventType() == 'vote_user'}
					{$aLang.stream_list_event_vote_user} <a href="{$oTarget->getUserWebPath()}" class="act">{$oTarget->getLogin()}</a>
				{elseif $oStreamEvent->getEventType() == 'join_blog'}
					{$aLang.stream_list_event_join_blog} <a href="{$oTarget->getUrlFull()}" class="act">{$oTarget->getTitle()|escape:'html'}</a>
				{elseif $oStreamEvent->getEventType() == 'add_friend'}
					{$aLang.stream_list_event_add_friend} <a href="{$oTarget->getUserWebPath()}" class="act">{$oTarget->getLogin()}</a>
				{else}
