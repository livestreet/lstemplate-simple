...
<div class="blog">
	<div id="vote_area_blog_{$oBlog->getId()}" class="voting {if $oBlog->getRating()>0}positive{elseif $oBlog->getRating()<0}negative{/if} {if !$oUserCurrent || $oBlog->getOwnerId()==$oUserCurrent->getId()}guest{/if} {if $oVote} voted {if $oVote->getDirection()>0}plus{elseif $oVote->getDirection()<0}minus{/if}{/if}">
	...
	<div class="blog-header">
        ...
        <div class="rss"><a href="{router page='rss'}blog/{$oBlog->getUrl()}/" class="rss" title="RSS"></a></div>
	</div>
		...
		{if $oUserCurrent and ($oUserCurrent->getId()==$oBlog->getOwnerId() or $oUserCurrent->isAdministrator() or $oBlog->getUserIsAdministrator() )}
            ...
            <em></em><span></span>
				{if $oUserCurrent->isAdministrator()}
					<a href="#" title="{$aLang.blog_delete}" id="blog_delete_show">{$aLang.blog_delete}</a>
				{else}
            ...
            <div class="button2 blog-edit">
            <em></em><span></span>
				<a href="{router page='blog'}edit/{$oBlog->getId()}/" title="{$aLang.blog_edit}">{$aLang.blog_edit}</a>
            </div>
			</li>
		{/if}
	</ul>
