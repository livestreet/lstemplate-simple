...
function toolbarPos() {
	...
	if ($('#toolbar section').length) {
		if ($(document).width() <= 1100) {
			...
			$('#toolbar').css({'position': 'fixed', 'right': 0, 'top' : $(document).scrollTop() + 134, 'display': 'block'});
		} else {
			$('#container').removeClass('toolbar-margin');
			$('#toolbar').css({'position': 'fixed', 'left': $('#wrapper').offset().left + $('#wrapper').outerWidth() + 10, 'top': 134, 'display': 'block'});
		}
	}
};
